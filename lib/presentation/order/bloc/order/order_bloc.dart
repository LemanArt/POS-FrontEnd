...
class OrderBloc extends Bloc<OrderEvent, OrderState> {
  OrderBloc() : super(const _Success([], 0, 0, '', 0, 0, '')) {
    ...
    on<_SetKasirData>((event, emit) {
      var currentStates = state as _Success; // Pastikan state adalah _Success
      emit(const _Loading()); // Emit loading state
      ...
      emit(_Success(
        ...
        currentStates.totalPrice,
        currentStates.paymentMethod,
        currentStates.nominal,
        event.idKasir, // Gunakan idKasir dari event
        event.namaKasir, // Gunakan namaKasir dari event
      ));
    });
